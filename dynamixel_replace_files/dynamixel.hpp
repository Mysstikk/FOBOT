...
private:
  ...
  // lo creamos como vector para que puedan haber varios grupos
  std::vector<dynamixel::GroupFastBulkRead*> group_fast_bulk_reads_;
...
public:
  ...
  DxlError InitDxlComm(std::vector<uint8_t> id_arr, std::string port_name, std::string baudrate, float protocol_version);
