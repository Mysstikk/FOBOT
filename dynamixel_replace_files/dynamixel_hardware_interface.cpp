  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  // Comprobamos que se ha especificado la versión del protocolo
  if (info_.hardware_parameters.find("protocol_version") == info_.hardware_parameters.end()) {
    RCLCPP_ERROR_STREAM(logger_, "Required parameter 'protocol_version' not found in hardware parameters");
    return hardware_interface::CallbackReturn::ERROR;
  } 

  port_name_ = info_.hardware_parameters["port_name"];
  baud_rate_ = info_.hardware_parameters["baud_rate"];
  // Guardamos en una variable el protocolo especificado
  protocol_version_ = std::stof(info_.hardware_parameters["protocol_version"]);
  ...
  if (controller_id_.size() > 0) {
    for (int i = 0; i < 10; i++) {
      ...
      // Anyadimos la variable de protocol_version_ para enviarla a la funcion de inicializacion
      if (dxl_comm_->InitDxlComm(id_arr, port_name_, baud_rate_, protocol_version_) == DxlError::OK) {
