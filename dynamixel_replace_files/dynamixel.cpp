...
Dynamixel::~Dynamixel()
{
  ...
  for (auto g : group_fast_bulk_reads_) {
    delete g;
  }
  group_fast_bulk_reads_.clear();
  ...
  float protocol_version)
{
  ...
  // MODIFICACIÓN: Desactivar protocolos rápidos que AX-12A no soporta
  if (packet_handler_->getProtocolVersion() == 1.0){
...
DxlError Dynamixel::SetMultiDxlRead()
{
  if (packet_handler_->getProtocolVersion() == 1.0) {
    return DxlError::OK;
  } else if (packet_handler_->getProtocolVersion() == 2.0) {
...
{
  if (packet_handler_->getProtocolVersion() == 1.0) {
    // --- HACK PARA AX-12A: LECTURA SECUENCIAL ---
    // Los AX-12A no soportan BulkRead ni SyncRead. 
    // Forzamos la lectura manual dato por dato.
    
    DxlError final_result = DxlError::OK;
    
    for (auto & group : read_data_list_) { 
      uint8_t id = group.comm_id;
      
      // Recorremos cada dato que ROS quiere leer (Posición, Velocidad...)
      for (size_t i = 0; i < group.item_name.size(); i++) {
        std::string item_name = group.item_name[i];
        uint32_t read_val = 0;

        // Usamos ReadItem estándar (Instrucción 0x02) que SIEMPRE funciona
        if (ReadItem(id, item_name, read_val) == DxlError::OK) {
           
           // --- CONVERSIÓN DE DATOS (0-1023 -> Radianes) ---
           double unit_value;
           bool is_signed;
           
           // Intentamos usar la info del modelo para convertir unidades correctamente
           if (dxl_info_.GetDxlUnitValue(id, item_name, unit_value) && 
               dxl_info_.GetDxlSignType(id, item_name, is_signed)) {
                  *group.item_data_ptr_vec[i] = ConvertValueWithUnitInfo(
                      id, item_name, read_val, group.item_size[i], is_signed);
           } else {
              // Fallback manual si falla la info del modelo
              if (item_name == "Present Position") {
                  *group.item_data_ptr_vec[i] = dxl_info_.ConvertValueToRadian(id, read_val);
              } else if (item_name == "Present Velocity") {
                  // Conversión aproximada para velocity si falta info (0.111 rpm por unidad)
                   *group.item_data_ptr_vec[i] = (is_signed ? (int32_t)read_val : read_val) * 0.111 * 0.10472; 
              } else {
                  *group.item_data_ptr_vec[i] = static_cast<double>(read_val);
              }
           }
        } else {
          // Si falla una lectura, no retornamos error fatal para que el bucle siga vivo
          // Solo imprimimos un aviso en consola si es necesario
          // fprintf(stderr, "Warn: Fallo lectura ID %d Item %s\n", id, item_name.c_str());
          final_result = DxlError::ITEM_READ_FAIL;
        }
      }
    }
...
{
  if (packet_handler_->getProtocolVersion() == 1.0){
    // --- HACK ESCRITURA AX-12A ---
    // Ignoramos Sync/Bulk Write. Enviamos las órdenes una a una.
    
    if (write_data_list_.empty()) {
      return DxlError::OK;
    }

    for (auto & group : write_data_list_) {
      uint8_t id = group.comm_id;

      for (size_t i = 0; i < group.item_name.size(); i++) {
        std::string item_name = group.item_name[i];
        double data_double = *group.item_data_ptr_vec[i]; // El valor que ROS quiere enviar (radianes)
        uint8_t size = group.item_size[i];
        uint32_t raw_value = 0;

        // 1. CONVERSIÓN: Radianes (ROS) -> 0-1023 (AX-12A)
        bool is_signed;
        double unit_value;
        
        // Intentamos usar la conversión oficial del SDK
        if (dxl_info_.GetDxlUnitValue(id, item_name, unit_value) && 
            dxl_info_.GetDxlSignType(id, item_name, is_signed)) 
        {
            raw_value = ConvertUnitValueToRawValue(id, item_name, data_double, size, is_signed);
        } 
        else 
        {
            // Fallback manual si falla la info del modelo
            if (item_name == "Goal Position") {
                raw_value = static_cast<uint32_t>(dxl_info_.ConvertRadianToValue(id, data_double));
            } else {
                raw_value = static_cast<uint32_t>(data_double);
            }
        }

        // 2. ENVÍO: Usamos la función WriteItem simple que sabemos que funciona
        // Ignoramos errores menores para no saturar el log
        WriteItem(id, item_name, raw_value);
      }
    }
...
DxlError Dynamixel::SetBulkReadItemAndHandler()
{
  ...
  group_fast_bulk_reads_.push_back(new dynamixel::GroupFastBulkRead(port_handler_, packet_handler_));
...
DxlError Dynamixel::SetFastBulkReadHandler(std::vector<uint8_t> id_arr)
{
  ...
  // Agrupar motores por {Direccion_Indirecta, Tamaño}
  std::map<std::pair<uint16_t, uint8_t>, std::vector<uint8_t>> memory_groups;
  ...
  for (auto it_id : id_arr){
    ...
    // Se agrupan los IDs
    memory_groups[{IN_ADDR, indirect_info_read_[it_id].size}].push_back(it_id);
  }
  // Se crea un GroupFastBulkRead por cada IDs
  for (const auto& group : memory_groups){
    uint16_t addr = group.first.first;
    uint8_t size = group.first.second;
    const auto& ids = group.second;
    
    auto new_group = new dynamixel::GroupFastBulkRead(port_handler_, packet_handler_);
    
    for (auto id : ids){
      if (new_group->addParam(id, addr, size)) {  // success
        fprintf(
          stderr, "[ID:%03d] Add BulkRead item : [Indirect Item Data][%d][%d]\n", id, addr, size);
      } else {
        fprintf(
    ...
    group_fast_bulk_reads_.push_back(new_group);
  }
  ...
  uint8_t id, std::string item_name, uint16_t item_addr, uint8_t item_size)
{
  ...
  if (!group_fast_bulk_reads_.empty() && group_fast_bulk_reads_.back()->addParam(id, item_addr, item_size) == true){
    fprintf(
...
DxlError Dynamixel::GetDxlValueFromBulkRead(double period_ms)
{
  ...
  /*if (use_fast_read_protocol_ && group_fast_bulk_read_ &&
    (fast_read_permanent_ || fast_read_fail_count_ < 10))*/
  if (use_fast_read_protocol_ && !group_fast_bulk_reads_.empty() &&
    (fast_read_permanent_ || fast_read_fail_count_ < 10))
  {
        ...
        if (CheckIndirectReadAvailable(id) != DxlError::OK) {
          ProcessDirectReadData(
            ...
            [this](uint8_t lambda_id, uint16_t addr, uint8_t size) {
              ...
              for (auto g : group_fast_bulk_reads_) {
                if (g->isAvailable(lambda_id, addr, size)){
                  return g->getData(lambda_id, addr, size);
                }
              }
              return 0U;
            });
  ...
  bool is_fast)
{
  ...
  if (is_sync) {
    ...
    if (is_fast && !group_fast_bulk_reads_.empty()) {
      for (auto g : group_fast_bulk_reads_) {
        dxl_comm_result = g->txRxPacket();
  ...
  if (is_sync) {
    ...
    if (is_fast && !group_fast_bulk_reads_.empty()) {
      // no se hace nada por que se hace en el txRx de arriba
    } else if (group_bulk_read_) {
